...
def check_assertion(assertion, output_dir):
    ...
    if atype == "file_exists":
        ...
        # e.g. { "type": "glob_exists", "pattern": ".specs/dags/*.md" }
        ...
        # e.g. { "type": "glob_grep", "glob": ".specs/dags/*.md", "pattern": "agy" }
        ...
        filename = assertion.get("file", "spec.md")
        search_pattern = os.path.join(output_dir, ".specs", "**", filename)
        matches = glob.glob(search_pattern, recursive=True)
        passed = False
        evidence = f"Not found: {filename}"
        if matches:
            for m in matches:
                with open(m, "r", errors="replace") as f:
                    content = f.read()
                    if re.search(assertion.get("pattern", ""), content):
                        passed = True
                        evidence = f"Matched in {os.path.basename(os.path.dirname(m))}/{filename}"
                        break
            if not passed:
                evidence = f"Pattern not matched in {len(matches)} file(s)"
        return {"text": f"Spec {filename} contains {assertion.get('pattern')}", "passed": passed, "evidence": evidence}

    elif atype == "socratic_turns":
        # Checks the socratic session had at least N turns (agent asked questions).
        # We need the mode to find the right transcript, but check_assertion doesn't receive mode right now.
        # However, the output_dir is scenario_id/with_dag_flow, so dirname is scenario_id.
        ...
        if os.path.exists(transcript_path):
            with open(transcript_path, "r", errors="replace") as f:
                ...
                if re.search(assertion.get("pattern", ""), content):
                    passed = True
                    evidence = f"Matched {assertion.get('pattern')}"
                else:
                    evidence = f"Pattern not found in transcript"
        return {"text": f"Transcript contains {assertion.get('pattern')}", "passed": passed, "evidence": evidence}
