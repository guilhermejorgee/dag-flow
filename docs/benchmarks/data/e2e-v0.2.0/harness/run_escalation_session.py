...
# Signals that indicate the agent has completed the Escalation phase.
FINISHED_SIGNALS = [
    "Benchmark Complete",
    "Escalation Complete",
    "All tasks Done",
...
_CHROME_RE = re.compile(
    ...
    r"|\[1\].*\[2\].*\[0\]|How.s the CLI experience" # Survey dialog
)

# Tool-call indicator lines (not questions, not content we care about).
_TOOL_CALL_RE = re.compile(r"^[●•]\s|^ListDir\(|^Read\(|^Create\(|^Write\(|^Edit\(|^context-mode/|^RunCommand\(|^DefineSubagent\(|^InvokeSubagent\(")
...
def find_question(screen: pyte.Screen, answered: set) -> tuple[str, str] | None:
    ...
    # Stable key = last question-containing segment (for deduplication)
    q_key = next((l for l in reversed(question_lines) if "?" in l), full_question)
...
def find_execution_request(screen: pyte.Screen, executed: set) -> tuple[str, str] | None:
    ...
    for line in lines[-15:]:
        ...
        if stripped.startswith(">") or stripped.startswith("○") or stripped.startswith("•") or stripped.startswith("action:") or "bash(" in stripped:
            continue
        
        # Check for run_dag.sh equivalent requests
        if "run_dag.sh" in stripped or ("run" in stripped and "dag" in stripped) or "run tasks" in stripped:
            if line not in executed:
                return line, "run_dag.sh"
                
        # Check for auditor.sh equivalent requests
        if "auditor.sh" in stripped or "double verification" in stripped or ("run" in stripped and "auditor" in stripped):
            if line not in executed:
                return line, "auditor.sh"
    return None
    ...
    with open(log_path, "wb") as raw_log:
        ...
        while (
            ...
            if result:
                ...
                # Check if agent is explicitly waiting for user input
                cursor_line = screen.display[cursor_row].strip()
                if cursor_line.startswith(">"):
                    exec_req = find_execution_request(screen, answered_questions)
                    if exec_req:
                        line, cmd_type = exec_req
                        answered_questions.add(line)
                        print(f"[escalation] Intercepted execution: {line}")
                        
                        if cmd_type == "run_dag.sh":
                            print("[escalation] Running run_dag.sh via subprocess...")
                            try:
                                words = line.split()
                                dag_arg = None
                                for w in words:
                                ...
                                import os
                                full_dag_path = os.path.join(workspace_dir, dag_arg)
                                if not os.path.exists(full_dag_path):
                                    answer = f"Error: The DAG file '{dag_arg}' does not exist in the workspace. Did you specify the correct path or forget to vault the file?"
                                else:
                                    res = subprocess.run(["bash", ".agents/skills/dag-flow/scripts/run_dag.sh", dag_arg], cwd=workspace_dir, capture_output=True, text=True, timeout=120)
                                    if res.returncode == 127:
                                        answer = "Error: run_dag.sh script not found. Is the skill initialized correctly?"
                                    elif res.returncode != 0:
                                        answer = "The execution of run_dag.sh failed. Please read the log file of the failed task in the `logs/` directory to diagnose the issue and initiate the escalation protocol."
                                    else:
                                        answer = "The execution of run_dag.sh completed successfully."
                            except subprocess.TimeoutExpired:
                                answer = "The execution timed out."
                        elif cmd_type == "auditor.sh":
                            print("[escalation] Intercepted auditor.sh")
                            answer = "auditor.sh returned PASS. The gate was successful."

                        print(f"[escalation] Emulator answers: {answer}")
                        transcript.append({
                            "interaction": interactions + 1,
                            "execution": cmd_type,
                            "answer": answer,
