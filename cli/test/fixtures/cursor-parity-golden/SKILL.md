...
- **Gate Execution:** After user approval, write the final spec (`spec.md`) and the Counted PAGRL (`spec.pagrl.xml`) strictly to `.specs/staging/[feature]/`. Then, you MUST explicitly use the `Shell` tool to execute `<path-to-skill>/scripts/commit_spec.sh [feature]`. This script runs Python validation and moves the files to the physically locked `.specs/features/` vault.
...
- **Reference:** You MUST use the `Read` tool to read [`references/design.md`](/references/design.md) before executing this phase. **Why:** the required-sections template and the counted PAGRL schema for Design live there. The depth of `design.md` is proportional to the architectural decisions the feature requires; the *shape* (which sections must appear) is uniform.
...
- **Gate Execution:** You MUST explicitly use the `Shell` tool to execute `<path-to-skill>/scripts/commit_design.sh [feature]`. This script runs Python validation and moves the files to the physically locked `.specs/features/` vault.
...
use `Read` to read `references/planner-template.md` verbatim - this is the Subagent
system prompt. Spawn a Subagent Planner (`Task` with `subagent_type=generalPurpose`) with a single `prompt` that contains:
...
- **Gate Execution:** Once you receive the JSON, write it to `.specs/staging/[feature]/dag.json`. You MUST explicitly use the `Shell` tool to execute `<path-to-skill>/scripts/write_dag.sh [feature] --phase tasks`. This runs Python validation on your XML and safely commits the DAG to `.specs/dags/`.
...
- **Escalation Protocol:** if the user reports a DAG failure, identify the failed task and read its log file in the `logs/` directory and read [`references/escalation-phase.md`](/references/escalation-phase.md) to initiate the recovery protocol. When instructed to update a task's status to Done, you MUST explicitly use the `Shell` tool to execute `python3 <path-to-skill>/scripts/update_task_status.py <dag_file> <task_id> Done`, as the vault is locked.
...
- **Artifact Generation & Gating:** After the Diagnosis PAGRL, spawn a Subagent Planner
  ...
  in the same staging folder. Then use `Shell` to execute
...
- You are **FORBIDDEN** from directly editing files in the physically locked `.specs/dags/` and `.specs/features/` directories. You MUST write to `.specs/staging/` and use `Shell` to invoke the `commit_*.sh` and `write_dag.sh` gate scripts.
