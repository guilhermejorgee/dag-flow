...
**USAGE:** The Orchestrator MUST load this file verbatim via `Read` and pass it as the
`system_prompt` to `Task`. Do NOT paraphrase or reconstruct from memory.
...
| **LLM Auditor (`cursor agent`)** | Semantic/architectural rules that cannot be shell-tested | SOLID, design patterns |
...
**FORBIDDEN:** using `cursor agent` to verify string presence, absence, replacement, or text refactoring.
...
Reserve `cursor agent` for semantic rules only.
...
[
  ...
  },
  {
    ...
    "done_when_gate": "cursor agent --print --output-format text --trust --force --approve-mcps -p \"Call ctx_index for src/routes/auth.js.\""
  }
]
</dag_json>
```
