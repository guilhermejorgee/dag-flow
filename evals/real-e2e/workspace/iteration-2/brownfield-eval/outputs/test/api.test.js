...
test('RBAC API Unit Tests', async (t) => {
  ...
  const superAdminToken = jwt.sign({ username: 'superadmin', role: 'SUPER-ADMIN' }, JWT_SECRET);

  await t.test('GET /super-admin-data (Super-Admin Only)', async (t) => {
    await t.test('should allow access to SUPER-ADMIN', async () => {
      const response = await request(app)
        .get('/super-admin-data')
        .set('Authorization', `Bearer ${superAdminToken}`);
      
      assert.strictEqual(response.status, 200);
      assert.strictEqual(response.body.message, 'Welcome Super-Admin');
    });

    await t.test('should deny access to ADMIN', async () => {
      const response = await request(app)
        .get('/super-admin-data')
        .set('Authorization', `Bearer ${adminToken}`);
  ...
  await t.test('DELETE /users/:id (Super-Admin Only)', async (t) => {
    await t.test('should allow SUPER-ADMIN to delete a user', async () => {
      // User with ID 2 exists in db.js
      const response = await request(app)
        .delete('/users/2')
        .set('Authorization', `Bearer ${superAdminToken}`);
    ...
    await t.test('should return 404 for non-existent user', async () => {
      const response = await request(app)
        .delete('/users/999')
        .set('Authorization', `Bearer ${superAdminToken}`);
