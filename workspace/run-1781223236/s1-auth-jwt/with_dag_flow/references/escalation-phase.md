...
2. **Diagnosis & Planning:** Orchestrator generates a Quick Mode Mini-DAG (`.specs/hotfixes/<task-id>/tasks.md`) specifically designed to fix the failure.
3. **Hotfix Execution:** Orchestrator instructs the **User** to run `<path-to-skill>/scripts/run_dag.sh <hotfix-tasks.md>`.
...
6. **Resolution:**
   - **If PASS:** Orchestrator explicitly updates the main task's status to `Done` in the main `tasks.md` file, then instructs the **User** to resume `<path-to-skill>/scripts/run_dag.sh <main-tasks.md>`.
