#!/usr/bin/env node
// caveman-stats — Cursor session stats (transcript_path or ~/.cursor/projects)

const fs = require('fs');
const path = require('path');
const os = require('os');
const { readFlag, appendFlag, readHistory, safeWriteFlag, getCursorDir, getFlagPath, getHistoryPath } = require('./caveman-config');
...
function findRecentSession(cursorDir) {
...
function parseCursorSession(filePath) {
  ...
  catch { return { outputTokens: 0, cacheReadTokens: 0, turns: 0, model: null, estimated: true }; }
  ...
  for (const line of raw.split('\n')) {
    ...
    if (entry.role !== 'assistant' || !entry.message) continue;
    turns++;
    for (const part of entry.message.content || []) {
      if (part.type === 'text' && part.text) outputChars += part.text.length;
    }
  }
  const outputTokens = Math.round(outputChars / 4);
  return { outputTokens, cacheReadTokens: 0, turns, model: null, estimated: true };
}

function parseSession(filePath) {
  const cursor = parseCursorSession(filePath);
  if (cursor.turns > 0) return cursor;
...
function formatStats({ outputTokens, cacheReadTokens, turns, mode, model, sessionPath, compressed, estimated }) {
  const sep = '──────────────────────────────────';
  ...
  return `\nCaveman Stats (Cursor)\n${sep}\n` +
    (shortPath ? `Session:  ${shortPath}\n` : '') +
    `Turns:    ${turns}\n${sep}\n` +
    `Output tokens:         ${outputTokens.toLocaleString()}${estimated ? ' (est. from transcript text)' : ''}\n` +
...
function main() {
  ...
  const sessionFile = sessionFileArg || findRecentSession(cursorDir);

  if (!sessionFile) {
    process.stderr.write('caveman-stats: no Cursor session transcript found.\n');
    process.exit(1);
  }
  ...
  if (parsed.turns > 0) {
    ...
    safeWriteFlag(path.join(cursorDir, '.caveman-statusline-suffix'), suffix);
  }

  if (share) {
    process.stdout.write(formatShare({ ...parsed, mode }) + '\n');
  } else {
    const scanDirs = [cursorDir, process.cwd()].filter((d, i, a) => a.indexOf(d) === i);
