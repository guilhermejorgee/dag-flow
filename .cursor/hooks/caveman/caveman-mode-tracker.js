#!/usr/bin/env node
// caveman — Cursor beforeSubmitPrompt tracker (/caveman, flag file, stats)

const fs = require('fs');
const path = require('path');
const { execFileSync } = require('child_process');
const { getDefaultMode, safeWriteFlag, readFlag, getFlagPath, VALID_MODES } = require('./caveman-config');
...
process.stdin.on('end', () => {
  try {
    ...
    // Cursor beforeSubmitPrompt cannot inject additional_context — flag side
    // effects only. Reinforcement comes from sessionStart + /caveman skill.
  } catch (e) {
    // Silent fail
  }
});
