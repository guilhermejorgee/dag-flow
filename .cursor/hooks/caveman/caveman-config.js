#!/usr/bin/env node
// caveman — Cursor runtime config (no Claude Code dependency)
//
...
function getFlagPath() {
  return path.join(getCursorDir(), '.caveman-active');
}

function getHistoryPath() {
  return path.join(getCursorDir(), '.caveman-history.jsonl');
}
...
// flag path (~/.cursor/.caveman-active) with a symlink to clobber other files.
//
// When the parent directory is itself a symlink (legitimate pattern: ~/.cursor
...
//   ln -s /opt/shared-cursor-config ~/.cursor
...
function safeWriteFlag(flagPath, content) {
  ...
  try {
    ...
    // This allows legitimate symlinked ~/.cursor dirs while still refusing
...
//
// Without this, a local attacker with write access to ~/.cursor/ could replace
...
// ($CURSOR_CONFIG_DIR/.caveman-history.jsonl).
//
