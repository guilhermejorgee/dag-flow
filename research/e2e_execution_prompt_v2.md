...
```bash
bash research/benchmarks/e2e-v0.1.0/harness/run_all.sh --mode dag_flow workspace/run-$(date +%s)
```
...
│  · Detecta conclusão via glob(.specs/**/tasks.md)  │
└──────────────────────┬──────────────────────────────┘
...
```bash
bash research/benchmarks/e2e-v0.1.0/harness/run_all.sh \
  --mode dag_flow \
      ...
      .specs/
        features/
          <feature-name>/   ← dag-flow scopes specs under features/<name>/
            design.md
            tasks.md        ← finish signal; triggers harness exit
...
> **Important**: dag-flow writes specs to `.specs/features/<feature-name>/` (not flat `.specs/`).  
...
- **`is_finished()` uses recursive glob** — `glob(.specs/**/tasks.md)` handles the  
  `.specs/features/<name>/tasks.md` path pattern used by dag-flow.
...
```bash
python3 research/benchmarks/e2e-v0.1.0/harness/create_scenario.py
```
