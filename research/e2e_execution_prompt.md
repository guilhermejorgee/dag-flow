...
- Use the provided bash scripts (e.g., `research/benchmarks/e2e-v0.1.0/scenarios/s1-auth-jwt/setup.sh`) to generate the isolated test workspaces for both `baseline` and `with_dag_flow`.
...
- Simply execute the master runner: `bash research/benchmarks/e2e-v0.1.0/harness/run_all.sh`
...
## Your Immediate Task
Execute the entire benchmark suite by running `bash research/benchmarks/e2e-v0.1.0/harness/run_all.sh`. Monitor the output and ensure the test subjects navigate the DAG-flow autonomously across all scenarios.
